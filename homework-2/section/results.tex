...
Per-query MRR@5 is zero unless the gold paper appears within the first five ranks and equals $1/r$ when the first relevant document appears at rank $r \le 5$. Confidence intervals reported in descriptive tables use the normal approximation over per-query scores and are explicitly labeled as descriptive summaries rather than inferential tests; they do not adjust for multiple comparisons.
...
\begin{table*}[htbp]
    ...
    \begin{tabular}{l p{4.1cm} r r l}
        ...
        Wilcoxon--Holm & \texttt{N1000} vs \texttt{N500} & -0.0008 & 0.0497 & Significant but negligible in size \\
        \bottomrule
    \end{tabular}%
    }
\end{table*}
