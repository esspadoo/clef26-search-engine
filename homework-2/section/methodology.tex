...
Early experiments focused on analyzer choices, stopword lists, and BM25 variants. These runs were useful for establishing a stable baseline, but they also showed that lexical overlap alone is insufficient for this task.
\newline
...
\end{itemize}
For each of these stages, we experimented with different models and kept the ones that performed best.

The following sections explain the four stages in detail.
...
Before settling on this query expansion strategy, we also tried two LLM models to see if they could improve our performance, \texttt{meta-llama/Llama-3.2-1B} and \texttt{openai/gpt-oss-20b}, with the following prompt:
\begin{promptbox}
    \small
    \ttfamily
    \begin{flushleft}
